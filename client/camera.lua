...
local function CreateInstuctionScaleform(scaleform)
    ...
    InstructionButtonMessage("Fermer la caméra")
    PopScaleformMovieFunctionVoid()
...
-- Events
RegisterNetEvent('ps-mdt:client:ActiveCamera', function(cameraId)
    ...
    elseif cameraId == 0 then
        ...
        ESX.ShowNotification("Cette caméra n'existe pas.", 5000, "error")
    end
end)

RegisterNetEvent('ps-mdt:client:DisableAllCameras', function()
    for k, _ in pairs(Config.SecurityCameras.cameras) do
        Config.SecurityCameras.cameras[k].isOnline = false
    end
end)

RegisterNetEvent('ps-mdt:client:EnableAllCameras', function()
    for k, _ in pairs(Config.SecurityCameras.cameras) do
        Config.SecurityCameras.cameras[k].isOnline = true
    end
end)

RegisterNetEvent('ps-mdt:client:SetCamera', function(key, isOnline)
    ...
    elseif type(key) == 'number' then
        ...
        error('ps-mdt:client:SetCamera did not receive the right type of key\nreceived type: ' .. type(key) .. '\nreceived value: ' .. key)
    end
end)
